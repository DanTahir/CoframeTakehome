...
async function main() {
  ...
  // --- markup source selection -------------------------------------------
  // A *rendered* capture has the page's own JS already applied, which bakes
  // runtime-generated NODES into the markup: clone hosts populated, headings
  // split into per-word spans, absolutely-positioned overlay clones left
  // mid-animation. Stripping runtime classes/styles does not remove those --
  // they ship as permanent copies AND get regenerated at runtime, so the
  // replica renders every animated element twice.
  //
  ...
  if (cfg.captureMode === 'static') {
    ...
    markupSource = staticUsable && staticRatio >= AUTO_STATIC_MIN_RATIO ? 'static' : 'rendered';
  }
  ...
  if (markupSource === 'rendered' && staticUsable && cfg.captureMode === 'auto') {
    console.log(
      '  NOTE: static markup looked incomplete (likely client-rendered), so the\n' +
        '        rendered DOM was used. Runtime-generated nodes may be baked in --\n' +
        '        check codegen.emptySelectors / codegen.unwrapSelectors.',
    );
  }
  ...
  if (!body) throw new Error(`No <body> in ${sourceFile} — re-run \`npm run capture\`.`);
  ...
  const manifest = {
    ...
    captureMode: cfg.captureMode,
    markupSource,
    sourceElementCounts: { static: staticCount, rendered: renderedCount },
  ...
  console.log(
    `  runtime clone hosts emptied  : ${stats.emptiedRuntimeHosts}` +
      ` (${stats.emptiedRuntimeNodes} baked node(s) removed)`,
  );
  console.log(`  runtime wrappers unwrapped   : ${stats.unwrappedRuntimeWrappers}`);
