...
function sanitizeTree(root) {
  ...
  const walk = (node) => {
    ...
    for (const child of children) {
      ...
      // Strip runtime-added classes.
      const clsAttr = child.attrs?.find((a) => a.name === 'class');
      if (clsAttr) {
        ...
        else child.attrs = child.attrs.filter((a) => a.name !== 'class');
      }
...
async function main() {
  ...
  const manifest = {
    ...
    source: 'scrape/raw/index.html (rendered, pre-scroll)',
