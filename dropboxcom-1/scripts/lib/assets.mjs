 ...
 *
 * These regex harvesters read RAW HTML, so an attribute written as
 * `srcset="...?id=x&amp;width=414"` yields a ref that still contains the
 * literal `&amp;`. Requesting that URL sends a query param actually named
 * `amp;width`, so the real `width` is silently absent — on an image-resizing
 * CDN that means asking for the full-resolution original, which such services
 * routinely refuse (Dropbox's returns 429). The asset then never lands on
 * disk, and codegen — which reads the same attribute back through a real HTML
 * parser, where entities ARE decoded — looks it up by the decoded URL, misses,
 * and leaves the remote CDN URL in the generated markup.
 *
