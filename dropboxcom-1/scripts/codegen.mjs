...
async function main() {
  ...
  //
  // The scraper harvests some URLs straight out of raw HTML source, so a
  // `srcset="...?id=x&amp;width=1280"` lands in the asset map with the literal
  // `&amp;` still in the key. codegen, by contrast, reads attributes back from
  // the parse5 tree, where entities are already decoded to `&`. The two forms
  // then never string-match, so `localAsset()` reports a miss for a file that
  // is sitting on disk and the generated markup keeps pointing at the CDN —
  // silently, since nothing 404s. On this site that was 154 of 167 leftover
  // remote refs (every multi-candidate srcset image), which fails the
